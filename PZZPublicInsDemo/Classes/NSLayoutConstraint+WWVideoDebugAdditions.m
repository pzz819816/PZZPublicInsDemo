//
//  NSLayoutConstraint+WWVideoDebugAdditions.m
//  WWVideoonry
//
...
#import "NSLayoutConstraint+WWVideoDebugAdditions.h"
#import "WWVideoConstraint.h"
#import "WWVideoLayoutConstraint.h"

@implementation NSLayoutConstraint (WWVideoDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(WWVideoLayoutPriorityDefaultHigh)      : @"high",
            @(WWVideoLayoutPriorityDefaultLow)       : @"low",
            @(WWVideoLayoutPriorityDefaultMedium)    : @"medium",
            @(WWVideoLayoutPriorityRequired)         : @"required",
            @(WWVideoLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(WWVideoLayoutPriorityDefaultHigh)                 : @"high",
            @(WWVideoLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(WWVideoLayoutPriorityDefaultMedium)               : @"medium",
            @(WWVideoLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(WWVideoLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(WWVideoLayoutPriorityDefaultLow)                  : @"low",
            @(WWVideoLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(WWVideoLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(WWVideo_key)] && [obj WWVideo_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj WWVideo_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != WWVideoLayoutPriorityRequired) {
